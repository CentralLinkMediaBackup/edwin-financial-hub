...
//
// This module is imported dynamically by useStore.initialize() ONLY when
// the accounts table is empty (first-ever app load on a fresh database).
// After seeding once, this code is never called again.

import { format, addWeeks } from 'date-fns'

function fmt(date) { return format(date, 'yyyy-MM-dd') }
...
function buildPaychecks() {
  ...
  const overrides = { 1: 993.21, 2: 1061.47 }
  for (let i = 0; i < 52; i++) {
    const d = addWeeks(start, i)
    rows.push({
      id: `paycheck-${i+1}`,
      date: fmt(d),
      amount: overrides[i+1] ?? 980,
...
const SEED_ACCOUNTS = [
  { key:'chaseDebit',      balance: 1971.32, updated_at: new Date().toISOString() },
  { key:'capitalOneDebit', balance: 0,       updated_at: new Date().toISOString() },
  { key:'cashApp',         balance: 0,       updated_at: new Date().toISOString() },
  { key:'paypal',          balance: 0,       updated_at: new Date().toISOString() },
]

const SEED_BILLS = [
  { id:'bill-1',  name:'Netflix',           amount:8.65,    due_day:23,  frequency:'monthly',   is_active:true, category:'Entertainment',  note:null,                 paid_this_month:false },
  { id:'bill-2',  name:'Amazon Prime',      amount:0.00,    due_day:7,   frequency:'monthly',   is_active:true, category:'Shopping',        note:null,                 paid_this_month:false },
  { id:'bill-3',  name:'TXU Electric',      amount:110.00,  due_day:26,  frequency:'monthly',   is_active:true, category:'Utilities',       note:null,                 paid_this_month:false },
  { id:'bill-4',  name:'Spectrum Internet', amount:50.26,   due_day:25,  frequency:'monthly',   is_active:true, category:'Utilities',       note:null,                 paid_this_month:false },
  { id:'bill-5',  name:'Spectrum Mobile',   amount:2.22,    due_day:19,  frequency:'monthly',   is_active:true, category:'Phone',            note:null,                 paid_this_month:false },
  { id:'bill-6',  name:'iPhone Data',       amount:55.00,   due_day:null,frequency:'monthly',   is_active:true, category:'Phone',            note:'Flexible due date',  paid_this_month:false },
  { id:'bill-7',  name:'Rent',              amount:1433.03, due_day:3,   frequency:'monthly',   is_active:true, category:'Housing',          note:'Due Jun 3',          paid_this_month:false },
  { id:'bill-8',  name:'Apt Insurance',     amount:19.17,   due_day:2,   frequency:'monthly',   is_active:true, category:'Insurance',        note:null,                 paid_this_month:false },
  { id:'bill-9',  name:'Amazon Kindle',     amount:12.98,   due_day:21,  frequency:'monthly',   is_active:true, category:'Entertainment',   note:null,                 paid_this_month:false },
  { id:'bill-10', name:'Spotify',           amount:14.06,   due_day:21,  frequency:'monthly',   is_active:true, category:'Entertainment',   note:null,                 paid_this_month:false },
  { id:'bill-11', name:'Capital One CR',    amount:61.00,   due_day:17,  frequency:'monthly',   is_active:true, category:'Debt',             note:null,                 paid_this_month:false },
  { id:'bill-12', name:'Chase CR',          amount:40.00,   due_day:6,   frequency:'monthly',   is_active:true, category:'Debt',             note:null,                 paid_this_month:false },
  { id:'bill-13', name:'Car Payment',       amount:530.00,  due_day:15,  frequency:'monthly',   is_active:true, category:'Transport',        note:null,                 paid_this_month:false },
  { id:'bill-14', name:'Marcus Medicine',   amount:50.00,   due_day:23,  frequency:'monthly',   is_active:true, category:'Health',           note:null,                 paid_this_month:false },
  { id:'bill-15', name:'Marcus Food',       amount:50.87,   due_day:16,  frequency:'bimonthly', is_active:true, category:'Food',             note:'Every 2 months',     paid_this_month:false },
  { id:'bill-17', name:'College',           amount:135.00,  due_day:28,  frequency:'monthly',   is_active:true, category:'Education',        note:'Before EOM',         paid_this_month:false },
  { id:'bill-18', name:'IONOS',             amount:1.00,    due_day:21,  frequency:'monthly',   is_active:true, category:'Business',         note:null,                 paid_this_month:false },
  { id:'bill-19', name:'Uber Subscription', amount:9.99,    due_day:21,  frequency:'monthly',   is_active:true, category:'Transport',        note:null,                 paid_this_month:false },
]

const SEED_TILT_LOGS = [{
...
const SEED_PENDING_INCOME = [{
  id:'pending-1', label:"Wife's Due", amount:224.57,
  details:[
  ...
  status:'pending', created_at:'2026-06-02',
}]
...
async function upsertTable(sb, table, rows, conflictCol = 'id') {
  if (!rows.length) return
  const { error } = await sb.from(table).upsert(rows, { onConflict: conflictCol })
  if (error) console.error(`[Seed] ${table}:`, error.message)
  else console.log(`[Seed] ${table}: ${rows.length} rows ✓`)
}

export async function seedAll(sb) {
  await upsertTable(sb, 'accounts',      SEED_ACCOUNTS,      'key')
  await upsertTable(sb, 'bills',         SEED_BILLS)
  await upsertTable(sb, 'tilt_logs',     SEED_TILT_LOGS)
  await upsertTable(sb, 'earnin_logs',   SEED_EARNIN_LOGS)
  await upsertTable(sb, 'afterpay_items',SEED_AFTERPAY)
  await upsertTable(sb, 'debts',         SEED_DEBTS)
  await upsertTable(sb, 'transactions',  SEED_TRANSACTIONS)
  await upsertTable(sb, 'paychecks',     buildPaychecks())
  // settings and pending_income require running 002_reset_and_correct_schema.sql first
  const { error: se } = await sb.from('settings').upsert(SEED_SETTINGS, { onConflict: 'id' })
  if (se) console.warn('[Seed] settings skipped (table may not exist yet):', se.message)
  else console.log('[Seed] settings: 1 row ✓')
  const { error: pe } = await sb.from('pending_income').upsert(SEED_PENDING_INCOME)
  if (pe) console.warn('[Seed] pending_income skipped (table may not exist yet):', pe.message)
  else console.log('[Seed] pending_income: 1 row ✓')
}
